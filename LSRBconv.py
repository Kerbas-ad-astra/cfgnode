...
def convert(file, LSnode):
    ...
    headNode = LSnode.GetNode("@KSCSWITCHER:FOR[RealSolarSystem]")
    launchsites = headNode.GetNode("@LaunchSites")
    llstring = "launchSites = ["
    for launchsite in launchsites.GetNodes("Site"):
        lsname = launchsite.GetValue("name")
        lslat = launchsite.GetNode("PQSCity").GetValue("latitude")
        lslon = launchsite.GetNode("PQSCity").GetValue("longitude")
        beacon = ConfigNode()
        beacon.AddValue("name",lsname)
        beacon.AddValue("latitude",lslat)
        beacon.AddValue("longitude",lslon)
        beacon.AddValue("range",500000)
